...
def main():
    ...
    quaternion_c = np.c_[dd_sca1b_c.q1.compute().to_numpy()[0: 3000],
                         dd_sca1b_c.q2.compute().to_numpy()[0: 3000],
                         dd_sca1b_c.q3.compute().to_numpy()[0: 3000],
                         dd_sca1b_c.q4.compute().to_numpy()[0: 3000]]
    quaternion_d = np.c_[dd_sca1b_d.q1.compute().to_numpy()[0: 3000],
                         dd_sca1b_d.q2.compute().to_numpy()[0: 3000],
                         dd_sca1b_d.q3.compute().to_numpy()[0: 3000],
                         dd_sca1b_d.q4.compute().to_numpy()[0: 3000]]
    biased_range = dd_kbr1b.biased_range.compute().to_numpy()[0: 600]
    ...
    alt_angle_sin = np.deg2rad(2 + 1 * np.sin(2. * np.pi / 250. * dd_sca1b_c.gps_time.compute().to_numpy()[0: 3000]))
    ...
    intersatellite = np.c_[
        dd_gni1b_c.xpos.compute().to_numpy()[0: 600] - dd_gni1b_d.xpos.compute().to_numpy()[0: 600],
        dd_gni1b_c.ypos.compute().to_numpy()[0: 600] - dd_gni1b_d.ypos.compute().to_numpy()[0: 600],
        dd_gni1b_c.zpos.compute().to_numpy()[0: 600] - dd_gni1b_d.zpos.compute().to_numpy()[0: 600]]
    ...
    # plt.show()
    np.savetxt(fname='..//..//temp//simu3000//ant_phase_centre_corr_2degree.txt', fmt='%.18e', X=phase_centre_proj)
    ...
    yf_plot = 2.0 / 600 * np.abs(yf[0: 600//2])
    xf = fftfreq(600, 5)[:600//2]
    plt.loglog(xf, yf_plot)
    plt.show()
    print(yf_plot[np.isclose(xf, 1 / 250)])
    print(yf_plot[np.isclose(xf, 2 / 250)])

    np.savetxt(fname='..//..//temp//simu3000//KBR1B_2019-01-01_X_53.txt', header="End of YAML header",
               X=np.c_[
                   dd_kbr1b.gps_time.compute().to_numpy()[0: 600],
                   dd_sca1b_d.sca_id.compute().to_numpy()[0: 600],
                   range + phase_centre_proj,
                   dd_kbr1b.range_rate.compute().to_numpy()[0: 600],
                   dd_kbr1b.range_accl.compute().to_numpy()[0: 600],
                   dd_kbr1b.iono_corr.compute().to_numpy()[0: 600],
                   dd_kbr1b.lighttime_corr.compute().to_numpy()[0: 600]
               ],
               fmt='%s')

    np.savetxt(fname='..//..//temp//simu3000//SCA1B_2019-01-01_A_05.txt', header='End of YAML header',
               X=np.c_[
                   dd_sca1b_c['gps_time'].compute().to_numpy()[0: 3000],
                   dd_sca1b_c['sca_id'].compute().to_numpy()[0: 3000],
                   dd_sca1b_c['GRACEFO_id'].compute().to_numpy()[0: 3000],
                   dd_sca1b_c['sca_id'].compute().to_numpy()[0: 3000],
                   quat_c
               ],
               fmt='%s')

    np.savetxt(fname='..//..//temp//simu3000//SCA1B_2019-01-01_B_05.txt', header="End of YAML header",
               X=np.c_[
                   dd_sca1b_d['gps_time'].compute().to_numpy()[0: 3000],
                   dd_sca1b_d['sca_id'].compute().to_numpy()[0: 3000],
                   dd_sca1b_d['GRACEFO_id'].compute().to_numpy()[0: 3000],
                   dd_sca1b_d['sca_id'].compute().to_numpy()[0: 3000],
                   dd_sca1b_d['q1'].compute().to_numpy()[0: 3000],
                   dd_sca1b_d['q2'].compute().to_numpy()[0: 3000],
                   dd_sca1b_d['q3'].compute().to_numpy()[0: 3000],
                   dd_sca1b_d['q4'].compute().to_numpy()[0: 3000],
               ],
    ...
    np.savetxt(fname='..//..//temp//gfo3000//KBR1B_2019-01-01_X_53.txt', header="End of YAML header",
               X=np.c_[
                   dd_kbr1b.gps_time.compute().to_numpy()[0: 600],
                   dd_sca1b_d.sca_id.compute().to_numpy()[0: 600],
                   dd_kbr1b.biased_range.compute().to_numpy()[0: 600] + phase_centre_proj,
                   dd_kbr1b.range_rate.compute().to_numpy()[0: 600],
                   dd_kbr1b.range_accl.compute().to_numpy()[0: 600],
                   dd_kbr1b.iono_corr.compute().to_numpy()[0: 600],
                   dd_kbr1b.lighttime_corr.compute().to_numpy()[0: 600]
    ...
    np.savetxt(fname='..//..//temp//gfo3000//SCA1B_2019-01-01_A_05.txt', header='End of YAML header',
               X=np.c_[
                   dd_sca1b_c['gps_time'].compute().to_numpy()[0: 3000],
                   dd_sca1b_c['sca_id'].compute().to_numpy()[0: 3000],
                   dd_sca1b_c['GRACEFO_id'].compute().to_numpy()[0: 3000],
    ...
    np.savetxt(fname='..//..//temp//gfo3000//SCA1B_2019-01-01_B_05.txt', header="End of YAML header",
               X=np.c_[
                   dd_sca1b_d['gps_time'].compute().to_numpy()[0: 3000],
                   dd_sca1b_d['sca_id'].compute().to_numpy()[0: 3000],
                   dd_sca1b_d['GRACEFO_id'].compute().to_numpy()[0: 3000],
