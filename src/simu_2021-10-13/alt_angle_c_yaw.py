...
def main():
    ...
    intersatellite = np.c_[
        dd_gni1b_c.xpos.compute().to_numpy()[0: 86400] - dd_gni1b_d.xpos.compute().to_numpy()[0: 86400],
    ...
    # plt.plot(phase_centre_proj[0: 1000])x
    ...
    freq, psd = welch(
        dd_kbr1b.biased_range.compute().to_numpy() - range[::5] + dd_kbr1b.lighttime_corr.compute().to_numpy() + phase_centre_proj,
    ...
    eqa = np.zeros([intersatellite.__len__(), 6])
    for index, (inter, rotm1, rotm2) in enumerate(zip(intersatellite[::5], rotm_A_t[::5], rotm_B_t[::5])):
        eqa[index, : 3] = np.matmul(inter / np.linalg.norm(inter), rotm1)
        eqa[index, 3:] = np.matmul(inter / np.linalg.norm(inter), rotm2)
    ...
    np.savetxt(fname='..//..//temp//normal1//KBR1B_2019-01-02_X_01.txt', header="End of YAML header",
               X=np.c_[
                   dd_kbr1b.gps_time.compute().to_numpy()[0: 200],
                   dd_sca1b_d.sca_id.compute().to_numpy()[0: 200],
                   range[0: 200] + phase_centre_proj[0: 200],
                   dd_kbr1b.range_rate.compute().to_numpy()[0: 200],
                   dd_kbr1b.range_accl.compute().to_numpy()[0: 200],
                   dd_kbr1b.iono_corr.compute().to_numpy()[0: 200],
                   dd_kbr1b.lighttime_corr.compute().to_numpy()[0: 200]
               ],
               fmt='%s')

    np.savetxt(fname='..//..//temp//normal1//SCA1B_2019-01-02_A_01.txt', header='End of YAML header',
               X=np.c_[
                   dd_sca1b_c['gps_time'].compute().to_numpy()[0: 1000],
                   dd_sca1b_c['sca_id'].compute().to_numpy()[0: 1000],
                   dd_sca1b_c['GRACEFO_id'].compute().to_numpy()[0: 1000],
                   dd_sca1b_c['sca_id'].compute().to_numpy()[0: 1000],
                   quat_c[0: 1000]
               ],
               fmt='%s')

    np.savetxt(fname='..//..//temp//normal1//SCA1B_2019-01-02_B_01.txt', header="End of YAML header",
               X=np.c_[
                   dd_sca1b_d['gps_time'].compute().to_numpy()[0: 1000],
                   dd_sca1b_d['sca_id'].compute().to_numpy()[0: 1000],
                   dd_sca1b_d['GRACEFO_id'].compute().to_numpy()[0: 1000],
                   dd_sca1b_d['sca_id'].compute().to_numpy()[0: 1000],
                   dd_sca1b_d['q1'].compute().to_numpy()[0: 1000],
                   dd_sca1b_d['q2'].compute().to_numpy()[0: 1000],
                   dd_sca1b_d['q3'].compute().to_numpy()[0: 1000],
                   dd_sca1b_d['q4'].compute().to_numpy()[0: 1000],
               ],
               fmt='%s')
    ...
    # np.savetxt(fname='..//..//temp//SCA1B_2019-01-02_A_05_c2.txt', header='End of YAML header',
    #           X=np.c_[dd_sca1b_c['gps_time'].compute().to_numpy()[1000: 1000],
    #                   dd_sca1b_c['GRACEFO_id'].compute().to_numpy()[1000: 1000],
    #                   dd_sca1b_c['sca_id'].compute().to_numpy()[1000: 1000],
    #                   quat_c],
    #           fmt='%s')
...
def load_data():
    dd_gni1b_c = dd.read_csv(urlpath='..//..//..//..//gracefo_dataset//gracefo_1B_2019-01-02_RL04.ascii.noLRI//GNI1B_2019-01-02_C_04.txt',
                             ...
                             header=None,
                             skiprows=148,
                             names=['gps_time', 'gracefo_id', 'coor_id', 'xpos', 'ypos', 'zpos', 'vx', 'vy', 'vz'],)
    ...
    dd_sca1b_c = dd.read_csv(urlpath='..//..//..//..//gracefo_dataset//gracefo_1B_2019-01-02_RL04.ascii.noLRI//SCA1B_2019-01-02_C_04.txt',
                             engine='c',
                             header=None,
                             sep='\s+',
                             skiprows=114,
                             ...
                             dtype={'q1': np.longdouble, 'q2': np.longdouble, 'q3': np.longdouble,
                                    'q4': np.longdouble})
    dd_sca1b_d = dd.read_csv(urlpath='..//..//..//..//gracefo_dataset//gracefo_1B_2019-01-02_RL04.ascii.noLRI//SCA1B_2019-01-02_D_04.txt',
                             engine='c',
                             header=None,
                             sep='\s+',
                             skiprows=114,
