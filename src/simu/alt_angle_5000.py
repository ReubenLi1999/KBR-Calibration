...
def main():
    ...
    quaternion_c = np.c_[dd_sca1b_c.q1.compute().to_numpy()[0: 5000],
                         dd_sca1b_c.q2.compute().to_numpy()[0: 5000],
                         dd_sca1b_c.q3.compute().to_numpy()[0: 5000],
                         dd_sca1b_c.q4.compute().to_numpy()[0: 5000]]
    quaternion_d = np.c_[dd_sca1b_d.q1.compute().to_numpy()[0: 5000],
                         dd_sca1b_d.q2.compute().to_numpy()[0: 5000],
                         dd_sca1b_d.q3.compute().to_numpy()[0: 5000],
                         dd_sca1b_d.q4.compute().to_numpy()[0: 5000]]
    biased_range = dd_kbr1b.biased_range.compute().to_numpy()[0: 1000]
    ...
    alt_angle_sin = np.deg2rad(2 + 1 * np.sin(2. * np.pi / 250. * dd_sca1b_c.gps_time.compute().to_numpy()[0: 5000]))
    ...
    intersatellite = np.c_[
        dd_gni1b_c.xpos.compute().to_numpy()[0: 1000] - dd_gni1b_d.xpos.compute().to_numpy()[0: 1000],
        dd_gni1b_c.ypos.compute().to_numpy()[0: 1000] - dd_gni1b_d.ypos.compute().to_numpy()[0: 1000],
        dd_gni1b_c.zpos.compute().to_numpy()[0: 1000] - dd_gni1b_d.zpos.compute().to_numpy()[0: 1000]]
    ...
    # plt.show()
    np.savetxt(fname='..//..//temp//simu5000//ant_phase_centre_corr_2degree_c2.txt', fmt='%.18e', X=phase_centre_proj)
    ...
    yf_plot = 2.0 / 1000 * np.abs(yf[0: 1000//2])
    xf = fftfreq(1000, 5)[:1000//2]
    plt.loglog(xf, yf_plot)
    plt.show()
    print(yf_plot[np.isclose(xf, 1 / 250)])
    print(yf_plot[np.isclose(xf, 2 / 250)])

    np.savetxt(fname='..//..//temp//simu5000//KBR1B_2019-01-01_X_55.txt', header="End of YAML header",
               X=np.c_[
                   dd_kbr1b.gps_time.compute().to_numpy()[0: 1000],
                   dd_sca1b_d.sca_id.compute().to_numpy()[0: 1000],
                   range + phase_centre_proj,
                   dd_kbr1b.range_rate.compute().to_numpy()[0: 1000],
                   dd_kbr1b.range_accl.compute().to_numpy()[0: 1000],
                   dd_kbr1b.iono_corr.compute().to_numpy()[0: 1000],
                   dd_kbr1b.lighttime_corr.compute().to_numpy()[0: 1000]
               ],
               fmt='%s')

    np.savetxt(fname='..//..//temp//simu5000//SCA1B_2019-01-01_A_05.txt', header='End of YAML header',
               X=np.c_[
                   dd_sca1b_c['gps_time'].compute().to_numpy()[0: 5000],
                   dd_sca1b_c['sca_id'].compute().to_numpy()[0: 5000],
                   dd_sca1b_c['GRACEFO_id'].compute().to_numpy()[0: 5000],
                   dd_sca1b_c['sca_id'].compute().to_numpy()[0: 5000],
                   quat_c
               ],
               fmt='%s')

    np.savetxt(fname='..//..//temp//simu5000//SCA1B_2019-01-01_B_05.txt', header="End of YAML header",
               X=np.c_[
                   dd_sca1b_d['gps_time'].compute().to_numpy()[0: 5000],
                   dd_sca1b_d['sca_id'].compute().to_numpy()[0: 5000],
                   dd_sca1b_d['GRACEFO_id'].compute().to_numpy()[0: 5000],
                   dd_sca1b_d['sca_id'].compute().to_numpy()[0: 5000],
                   dd_sca1b_d['q1'].compute().to_numpy()[0: 5000],
                   dd_sca1b_d['q2'].compute().to_numpy()[0: 5000],
                   dd_sca1b_d['q3'].compute().to_numpy()[0: 5000],
                   dd_sca1b_d['q4'].compute().to_numpy()[0: 5000],
               ],
    ...
    np.savetxt(fname='..//..//temp//gfo5000//KBR1B_2019-01-01_X_55.txt', header="End of YAML header",
               X=np.c_[
                   dd_kbr1b.gps_time.compute().to_numpy()[0: 1000],
                   dd_sca1b_d.sca_id.compute().to_numpy()[0: 1000],
                   dd_kbr1b.biased_range.compute().to_numpy()[0: 1000] + phase_centre_proj,
                   dd_kbr1b.range_rate.compute().to_numpy()[0: 1000],
                   dd_kbr1b.range_accl.compute().to_numpy()[0: 1000],
                   dd_kbr1b.iono_corr.compute().to_numpy()[0: 1000],
                   dd_kbr1b.lighttime_corr.compute().to_numpy()[0: 1000]
    ...
    np.savetxt(fname='..//..//temp//gfo5000//SCA1B_2019-01-01_A_05.txt', header='End of YAML header',
               X=np.c_[
                   dd_sca1b_c['gps_time'].compute().to_numpy()[0: 5000],
                   dd_sca1b_c['sca_id'].compute().to_numpy()[0: 5000],
                   dd_sca1b_c['GRACEFO_id'].compute().to_numpy()[0: 5000],
    ...
    np.savetxt(fname='..//..//temp//gfo5000//SCA1B_2019-01-01_B_05.txt', header="End of YAML header",
               X=np.c_[
                   dd_sca1b_d['gps_time'].compute().to_numpy()[0: 5000],
                   dd_sca1b_d['sca_id'].compute().to_numpy()[0: 5000],
                   dd_sca1b_d['GRACEFO_id'].compute().to_numpy()[0: 5000],
