...
!   * Inspired by: [EasyPlot](https://pypi.python.org/pypi/EasyPlot)
    ...
    character(len=*), parameter :: real_fmt_default = '(E33.16)' !! default real number format string
    integer, parameter          :: max_real_len     = 33         !! max string length for reals
...
! Add an x,y plot.
    ...
    integer(ip),            intent (out)          :: istat        !! status output (0 means no problems)
...
!  * modified: Jacob Williams 6/16/2017
    ...
    subroutine savefig(me, figfile, pyfile, ismat, dpi, transparent, facecolor, edgecolor, orientation, istat)

    class(pyplot),    intent(inout)        :: me          !! pyplot handler
    character(len=*), intent(in)           :: figfile     !! file name for the figure
    character(len=*), intent(in), optional :: pyfile      !! name of the Python script to generate
    logical         , intent(in), optional :: ismat
