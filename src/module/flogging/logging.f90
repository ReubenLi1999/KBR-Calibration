...
contains
  ...
  subroutine logger_message(this,source,priority,message)
    ...
    if (priority >= this%logfile_threshold) then
      ...
      this%fileunit = 1999
      open(unit=this%fileunit, file=this%logfile, iostat=ios, status="old", action="write", position='append')
      if ( ios /= 0 ) stop "Error opening file name"
      write(this%fileunit, default_format) current_time(), source, output
      close(unit=this%fileunit, iostat=ios)
      if ( ios /= 0 ) stop "Error closing file unit this%fileunit"
